...
const url = 'mongodb://bot:ddddee6125919fe090550f0d7e31b88f@dokku-mongo-bot:27017/bot';
...
app.post('/activate', (req, res) => {
  Activate.find({}).then(data => {
    ...
    if (data.length < 1) {
      ...
      Activate.findByIdAndUpdate(data[0]._id, {
        activated: req.body.activate,
      }, { new: true })
        .then(data => {
          res.send("Successful");
...
app.post('/add', (req, res) => {
  ...
  Mbot.insertMany([
    { phone: req.body.phone, session: req.body.session },
...
(async () => {
  const stringSession = new StringSession('');
  ...
  client.addEventHandler((update) => {
    if(update.CONSTRUCTOR_ID == 1299050149 && update.SUBCLASS_OF_ID == 2331323052){
      Group.findOne({})
      .then(ndata => {
        if(!ndata){
         
        }else{
          Message.findOne({message: update.message})
          .then(data => {
            if(data){
              
              client.sendMessage(ndata.id, { message: data.reply, replyTo: update.id });
            
            }else{
              
              client.sendMessage(ndata.id, { message: "There is no doubt about the legitimacy of this project", replyTo: 256 });
      ...
      Group.findOne({}, function(err, info) {
        ...
        (async () => {
              ...
              if (dataa[0].caption && dataa[0].image) {
                await client.sendFile(info.id, { file: dataa[0].image, caption: dataa[0].caption });
              } else if (dataa[0].caption && !dataa[0].image) {
                await client.sendMessage(info.id, { message: dataa[0].caption });
      ...
      Group.findOne({}, function(err, info) {
        ...
        (async () => {
              ...
              if (datasa[0].caption && datasa[0].image) {
                await client.sendFile(info.id, { file: datasa[0].image, caption: datasa[0].caption });
              } else if (datasa[0].caption && !datasa[0].image) {
                await client.sendMessage(info.id, { message: datasa[0].caption });
